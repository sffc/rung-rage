...
function recurse(gameState, initialLevel, level, cache) {
	...
	// Base Case 2
	if (level - initialLevel >= MAX_DEPTH) {
		return {
			score: gameState.getScore(false),
			move: null
		};
	...
	if (!cache[key]) {
		if (level % 2 == 0) {
			// Human Player
			cache[key] = recurseHelper(gameState, gameState.p1, 1, MAX_HUMAN_BREADTH, initialLevel, level, cache);
		} else {
			// Computer Player
			cache[key] = recurseHelper(gameState, gameState.p2, -1, MAX_COMPUTER_BREADTH, initialLevel, level, cache);
		}
	}
	return cache[key];
}

function recurseHelper(gameState, player, direction, breadth, initialLevel, level, cache) {
	...
	for (let playerMove of playerMoves) {
		...
		for (let rungMove of rungsMoves) {
			...
			const candidate = recurse(gameState, initialLevel, level + 1, cache);
...
function computeMove(gameState, initialLevel) {
	let candidate = recurse(gameState, initialLevel, initialLevel, {});
